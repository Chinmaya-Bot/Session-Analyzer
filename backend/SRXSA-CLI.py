...
_parser.add_option('--session',help='Display Sessions created and destroyed',
...
def analyze_session_table(sessiondata, numdisplayed):
	...
	if FLAGS.sessions:
		print("Total number of Sessions seen: %d" % total_sessions)
		print("Top %d Session Timeout:" %numdisplayed)
		print("Number of Connections	-	Session Timeout")
		print("-"*80)		
		for i,j in sessionl[:numdisplayed]:				
			pp = int(j)
			sessions_percent = pp/total_sessions
			print("%s	-	%s    (%s Percent)" %(pp, i, round(sessions_percent*100, 2)))
		print("\n"*2)
